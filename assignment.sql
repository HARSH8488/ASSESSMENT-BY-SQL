...
select e.last_name,d.department_id,e.job_id from employees e join departmentS D on e.department_id=d.department_id 
...
-- employees in department 2 are given a 10% raise,
--  employees in departments 4 and 5 are given a 15% raise, and 
-- employees in department 6 are not given a raise.
/*   1) */ 
...
SELECT e.last_name AS manager_name, e.salary AS manager_salary, d.department_name FROM employees e
JOIN departments d ON e.department_id = d.department_id WHERE e.job_id LIKE 'MAN%' ORDER BY e.salary DESC LIMIT 3;